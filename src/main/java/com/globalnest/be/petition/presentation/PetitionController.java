...
public class PetitionController {
    ...
    public ResponseEntity<ResponseTemplate<?>> getPetitionMainPage(
            ...
            @RequestBody PetitionSortRequest petitionSortRequest,
            @AuthenticationPrincipal CustomOAuth2User user
    ) {
        PetitionResponseList petitionResponseList =
                petitionService.findPetitionResponseList(page, size, petitionSortRequest);

        return ResponseEntity
                .status(HttpStatus.OK)
                .body(ResponseTemplate.from(
                        translationService.getChatResponse(petitionResponseList, user.getLanguage())));
    }
    ...
    public ResponseEntity<ResponseTemplate<?>> getPetitionDetail(
        ...
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(ResponseTemplate.from(
                        translationService.getChatResponse(petitionDetailResponse, user.getLanguage())));
    }
