...
public class PetitionRepositoryImpl implements PetitionRepositoryCustom {
    ...
    @Override
    public List<PetitionResponse> getPetitionResponses(Boolean includeExpired, PetitionType petitionType,
                                                       Boolean sortByAgreementCount, int page, int size) {
        // 조건 정의
        BooleanBuilder conditions = createPetitionConditions(includeExpired, petitionType);
        ...
        return queryFactory
            ...
            .orderBy(orderSpecifier(sortByAgreementCount)) // 정렬 조건 추가
    ...
    // 정렬 조건을 처리하는 메서드
    private OrderSpecifier<?>[] orderSpecifier(Boolean sortByAgreementCount) {
        List<OrderSpecifier<?>> orderSpecifiers = new ArrayList<>();

        // 동의자 수 기준으로 정렬
        if (sortByAgreementCount != null && sortByAgreementCount) {
    ...
    private BooleanBuilder createPetitionConditions(Boolean includeExpired, PetitionType petitionType) {
        ...
        if (petitionType != null) {
            conditions.and(petition.petitionType.eq(petitionType));
        }
